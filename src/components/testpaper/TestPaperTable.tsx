...
type TestPaperTableProps = {
   ...
   * 
   * 사용 패턴:
   * - 개별 체크박스 상태 결정: `selectedIds.has(paper.id)`
   * - 전체 선택 상태 계산: `selectedIds.size === papers.length`
   * - 부분 선택 상태 확인: `selectedIds.size > 0 && selectedIds.size < papers.length`
   * 
   * @example
   * ```typescript
   * // 선택 상태 관리 예시
   ...
   *     const currentPageIds = papers.map(paper => paper.id);
   *     setSelectedIds(new Set(currentPageIds));
   *     
   *     // 분석용 로깅
   *     analytics.track('test_papers_select_all', { 
   *       count: currentPageIds.length 
   *     });
   *   } else {
   *     // 모든 선택 해제
   *     setSelectedIds(new Set());
   *     analytics.track('test_papers_deselect_all');
   *   }
   * };
   * ```
   */
   ...
   *   
   *   // 인쇄 요청 분석
   *   analytics.track('test_paper_print_requested', {
   *     paperId: paper.id,
   *     paperName: paper.testName,
   ...
   *   
   *   // 시험지의 상세 문제 데이터 로드
   *   fetchTestPaperProblems(paper.id)
   *     .then(problems => {
   ...
   *       logError(error, 'TestPaperTable.handleOpenProblemModal');
   *     });
   * };
   * ```
   */
   ...
   *       securityLog.warn('answer_access_failed', { paperId: paper.id, error });
   *     });
   * };
   * ```
   */
   ...
   * 
   * 필드별 기본 동작:
   * - unitName/testName: 첫 클릭 시 asc (가나다순)
   ...
   *   
   *   // 정렬 기준 분석 트래킹
   *   analytics.track('test_papers_sorted', { field, order: newOrder });
   * };
   * ```
   */
