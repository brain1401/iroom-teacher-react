...
type TestPaperTableProps = {
  ...
  onOpenDetail: (paper: TestPaper) => void;
};

export function TestPaperTable({ papers, selectedIds, onSelectAll, onSelect, onOpenPrint, onOpenDetail }: TestPaperTableProps) {
        ...
        {papers.map((paper) => (
          <TableRow key={paper.id}>
            ...
            <TableCell className="flex gap-2">
              <Button variant="outline" size="sm">답안보기</Button>
              <Button variant="outline" size="sm">문제보기</Button>
              <Button variant="outline" size="icon" onClick={() => onOpenPrint(paper)}>
                <Printer className="h-4 w-4" />
              </Button>
            </TableCell>
            <TableCell>
              <Button size="sm" onClick={() => onOpenDetail(paper)}>시험지 상세</Button>
            </TableCell>
          </TableRow>
        ))}
      </TableBody>
