...
export function ExamFilterSidebar({
  ...
  const [difficultyFilters, setDifficultyFilters] = useAtom(difficultyFiltersAtom);
  ...
  const [participationRange, setParticipationRange] = useAtom(participationRateRangeAtom);
  ...
  const [advancedOptions, setAdvancedOptions] = useAtom(advancedSearchOptionsAtom);

  // 파생 상태
  const activeFiltersCount = useAtomValue(activeFiltersCountAtom);
  const resetAllFilters = useSetAtom(resetAllFiltersAtom);
  const [filterPresets, setFilterPresets] = useAtom(filterPresetsAtom);
   ...
   * 난이도 필터 핸들러
   */
  const handleDifficultyFilter = (difficulty: ExamLevel, checked: boolean) => {
  ...
  const applyPreset = (presetKey: string) => {
    ...
    setDifficultyFilters(preset.filters.difficultyFilters);
    setQuestionRange(preset.filters.questionCountRange);
    setParticipationRange(preset.filters.participationRateRange);
  };

  /**
   * 필터 콘텐츠 컴포넌트 (모바일/데스크톱 공통 사용)
   */
  const FilterContent = ({ isMobileMode = false }: { isMobileMode?: boolean }) => (
    <ScrollArea className={cn(isMobileMode ? "h-[70vh]" : "h-[600px]", isMobileMode ? "" : "pr-3")}>
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) =>
                    setAdvancedOptions((prev) => ({ ...prev, caseSensitive: checked }))
                  }
                />
              </div>
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) =>
                    setAdvancedOptions((prev) => ({ ...prev, exactMatch: checked }))
                  }
                />
              </div>
            </div>
        ...
        >
          ...
          <CollapsibleContent className="space-y-4 mt-3">
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-3">
                {(["기초", "보통", "심화"] as ExamLevel[]).map((difficulty) => (
                  <div key={difficulty} className="flex items-center space-x-3">
                    <Checkbox
                      id={`difficulty-${difficulty}`}
                      checked={difficultyFilters.includes(difficulty)}
                      onCheckedChange={(checked) => handleDifficultyFilter(difficulty, checked as boolean)}
                    />
                    <Label htmlFor={`difficulty-${difficulty}`} className="text-xs cursor-pointer">
                      {difficulty}
                    </Label>
                  </div>
                ))}
        ...
        >
          ...
          <CollapsibleContent className="space-y-4 mt-3">
            ...
            <div className="space-y-2">
              ...
              <div className={cn("grid gap-2", isMobileMode ? "grid-cols-1" : "grid-cols-2")}>
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" size="sm" className="justify-start text-xs h-9">
                      <CalendarIcon className="mr-1 h-3 w-3" />
                      {dateFilters.createdFrom
                        ? format(dateFilters.createdFrom, "MM/dd", { locale: ko })
                        : "시작일"}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0">
                    <Calendar
                      ...
                      onSelect={(date) => handleDateRangeChange("createdFrom", date)}
                      initialFocus
                    />
                  </PopoverContent>
                </Popover>
                  ...
                  <PopoverTrigger asChild>
                    <Button variant="outline" size="sm" className="justify-start text-xs h-9">
                      ...
                      {dateFilters.createdTo
                        ? format(dateFilters.createdTo, "MM/dd", { locale: ko })
                        : "종료일"}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0">
                    <Calendar
                      ...
                      onSelect={(date) => handleDateRangeChange("createdTo", date)}
                      initialFocus
                    />
                  </PopoverContent>
                </Popover>
                  ...
                  <PopoverTrigger asChild>
                    <Button variant="outline" size="sm" className="justify-start text-xs h-9">
                      ...
                      {dateFilters.updatedFrom
                        ? format(dateFilters.updatedFrom, "MM/dd", { locale: ko })
                        : "시작일"}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0">
                    <Calendar
                      ...
                      onSelect={(date) => handleDateRangeChange("updatedFrom", date)}
                      initialFocus
                    />
                  </PopoverContent>
                </Popover>
                  ...
                  <PopoverTrigger asChild>
                    <Button variant="outline" size="sm" className="justify-start text-xs h-9">
                      ...
                      {dateFilters.updatedTo
                        ? format(dateFilters.updatedTo, "MM/dd", { locale: ko })
                        : "종료일"}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0">
                    <Calendar
                      ...
                      onSelect={(date) => handleDateRangeChange("updatedTo", date)}
                      initialFocus
                    />
                  </PopoverContent>
                </Popover>
          ...
          <CollapsibleContent className="space-y-4 mt-3">
            ...
            <div className="space-y-3">
              ...
              <Slider
                ...
                onValueChange={([min, max]) => setParticipationRange({ min, max })}
                max={100}
                min={0}
                step={5}
                className="w-full"
                  ...
                  {availableUnits.map((unit) => (
                    <div key={unit} className="flex items-center space-x-3">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => handleUnitFilter(unit, checked as boolean)}
                      />
                      <Label htmlFor={`unit-${unit}`} className="text-xs cursor-pointer line-clamp-1">
                        {unit}
                      </Label>
                    </div>
                  ))}
                  ...
                  {availableExamTypes.map((type) => (
                    <div key={type} className="flex items-center space-x-3">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => handleExamTypeFilter(type, checked as boolean)}
                      />
      ...
      <Sheet open={showSidebar} onOpenChange={setShowSidebar}>
        <SheetContent side="left" className="w-full max-w-sm p-0">
          ...
          <div className="flex-1 px-4">
            ...
            <FilterContent isMobileMode={true} />
          </div>
        </SheetContent>
      </Sheet>
    );
