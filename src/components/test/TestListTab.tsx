...
type TestListTabProps = {
   ...
   * - selectedTestId와 함께 사용하여 선택된 시험 정보 표시
   * - 파란색 배경의 알림 박스에 표시
 ...
 *
 * 설계 원칙:
 * - 중앙 집중식 상태 관리: useTestList 훅을 통한 일관된 상태 관리
 ...
 * └── Dialog (상세 정보 모달)
 *     └── TestSubmissionTable (제출 현황)
 * ```
 *
 * 상태 관리:
 * - useTestList 훅을 통한 중앙집중식 상태 관리
 * - filteredPapers: 검색/필터링 적용된 시험 목록
 * - selectedIds: 사용자가 선택한 시험 ID들의 Set
 * - activeModal: 현재 활성화된 모달 상태 ("detail" | null)
 * - filters: 검색어, 검색 범위, 학년 필터 상태
 *
 * 검색 및 필터링:
 * - 검색 범위: "all"(시험명+단원명), "testName"(시험명), "unitName"(단원명)
 * - 학년 필터: 중1, 중2, 중3 개별 선택
 * - 실시간 검색: 타이핑과 동시에 결과 업데이트
 * - 복합 필터: 검색어와 학년 필터 조합 적용
 *
 * 대시보드 연동:
 * - selectedTestId/selectedTestName props로 외부 컨텍스트 수신
 ...
 * - 시험별 제출 현황 상세 보기
 * - TestSubmissionTable로 학생별 제출 정보 표시
 * - 스크롤 가능한 대용량 데이터 지원
 * - 키보드 ESC 및 외부 클릭으로 닫기 지원
 *
 * 성능 최적화:
 * - useTestList 훅의 메모이제이션 활용
 ...
 *
 * // 대시보드 연동 사용법
 * function TestManagementWithDashboard() {
 *   const [selectedTest, setSelectedTest] = useState<{id: string, name: string} | null>(null);
 *   
 *   return (
 *     <div className="flex">
 *       <DashboardSidebar onTestSelect={setSelectedTest} />
 ...
 *
 * // 커스텀 이벤트 핸들러와 함께 사용
 * function TestListWithCustomActions() {
 *   const handleTestSelect = (testId: string) => {
 *     console.log('선택된 시험:', testId);
 *     // 커스텀 로직 실행
 *   };
 *   
 *   return (
 *     <TestListTab
 *       selectedTestId="test-001"
 *       selectedTestName="중간고사 수학"
 *     />
 *   );
 * }
 * ```
 ...
 * 데이터 플로우:
 * ```
 * useTestList 훅 → 상태 및 액션 제공 →
 * TestListTab → Toolbar (필터링) →
 * TestTable (표시) → Dialog (상세) →
 * TestSubmissionTable (제출현황)
 * ```
 *
 * Props 전달 체계:
 * ```
 * 외부 → TestListTab (selectedTestId, selectedTestName)
 * TestListTab → Toolbar (검색/필터 상태 및 핸들러)
 * TestListTab → TestTable (목록 데이터 및 선택 핸들러)
 * TestListTab → Dialog (모달 상태 및 내용)
 * Dialog → TestSubmissionTable (제출 현황 데이터)
 * ```
 *
 * 스타일링:
