 ...
 */
type TestTableProps = {
  /** 시험 목록 데이터 배열 - Test 타입의 배열로 각 시험 정보를 포함 */
  papers: Test[];
 ...
 *
 * 설계 원칙:
 * - 데이터 중심 설계: Test 타입 기반의 완전한 시험 정보 표시
 ...
 * - isAllSelected: 전체 선택 상태 계산 (papers.length와 selectedIds.size 비교)
 ...
 * - 타입 안전한 라우팅 (testId 파라미터 전달)
 * - Search 파라미터로 testName 추가 전달
 ...
 *       onOpenPrint={(test) => console.log('인쇄:', test.id)}
 *       onOpenDetail={(test) => console.log('상세보기:', test.id)}
 *     />
 *   );
 * }
 *
 ...
 *   
 *   // 일괄 처리 로직
 *   processBulkAction(selectedTests);
 * };
 *
 * // 필터링된 목록 표시
 * const filteredTests = tests.filter(t => t.questionLevel === "상");
 * 
 * <TestTable
 *   papers={filteredTests}
 *   selectedIds={selectedIds}
 *   // ... 기타 props
 * />
 * ```
 *
 * 데이터 요구사항:
 * - Test 타입의 배열 (id, unitName, testName, questionCount, questionLevel, actualParticipants, totalParticipants 포함)
 * - 각 시험의 id는 고유해야 함 (키 및 라우팅용)
 * - questionLevel은 getDifficultyBadgeVariant에서 지원하는 값이어야 함
 * - 참여 현황 데이터는 숫자 타입이어야 함
 *
 * 라우팅 연동:
 * - /main/test-management/$testId 경로로 이동
 * - testId 파라미터와 testName 검색 파라미터 전달
