...
export default function EditClassDialog({ open, onOpenChange, value }: EditClassDialogProps) {
  const [classNameValue, setClassNameValue] = useState("");
  const [timeValue, setTimeValue] = useState("13:00-14:00");
  const [teacherValue, setTeacherValue] = useState("김다라");
  ...
  useEffect(() => {
    if (open && value) {
      setClassNameValue(value.className);
      setTimeValue(value.time);
      setTeacherValue(value.teacher);
      setGradeValue(value.grade);
      // 기존 time에서 요일과 시간 분리
      if (value.time) {
        const timeParts = value.time.split(" ");
        if (timeParts.length >= 2) {
          setDays([timeParts[0]]); // "월요일 13:00-14:00"에서 "월요일" 추출
          setTimeSlot(timeParts[1]); // "월요일 13:00-14:00"에서 "13:00-14:00" 추출
        }
      }
    }
  }, [open, value]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] bg-white">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold">반 정보 수정</DialogTitle>
        </DialogHeader>

        <div className="grid gap-4">
          <div className="grid gap-2">
            <label className="text-sm font-medium">반 이름</label>
            <Input value={classNameValue} onChange={(e) => setClassNameValue(e.target.value)} />
          </div>
