...
} from "@/data/test-paper-mock-data";
import type {UnitTreeNode, ProblemHierarchy} from "@/data/test-paper-mock-data";
 ...
 * - 시험 출제 완료 후 목록 이동
 */
export function useTestPaperRegistration() {
  // 문제지 정보 상태
  ...
  // 문제 상세보기 모달 상태
  const [isProblemDetailModalOpen, setIsProblemDetailModalOpen] = useState<boolean>(false);
  const [selectedProblemForDetail, setSelectedProblemForDetail] = useState<Problem | null>(null);

  // 시험 목록 탭 이동 콜백
  const [onTestCreated, setOnTestCreated] = useState<((test: Omit<Test, 'id' | 'createdAt' | 'updatedAt' | 'totalParticipants' | 'actualParticipants'>) => void) | null>(null);

  /**
   * 시험 목록 탭 이동 콜백 설정
   * @param callback 시험 생성 완료 후 호출될 콜백 함수
   */
  const setTestCreatedCallback = useCallback((callback: (test: Omit<Test, 'id' | 'createdAt' | 'updatedAt' | 'totalParticipants' | 'actualParticipants'>) => void) => {
    setOnTestCreated(() => callback);
  }, []);
   ...
   */
  const replaceProblem = useCallback((oldProblemId: string, newProblemId: string) => {
  ...
  const getSelectedProblems = useCallback((): Problem[] => {
    ...
    const orderedProblemIds = problemOrder.length > 0 
   ...
   */
  const handleCreateTestPaper = useCallback(() => {
    // 시험 정보 생성
    const newTest: Omit<Test, 'id' | 'createdAt' | 'updatedAt' | 'totalParticipants' | 'actualParticipants'> = {
      unitName: `${selectedGrade} 수학 - 문제지`,
      testName: examName,
      questionCount: totalQuestionCount,
      questionLevel: totalQuestionCount > 25 ? "심화" : totalQuestionCount > 20 ? "보통" : "기초",
      status: "승인대기",
    };
   ...
   * 문제지 미리보기 핸들러
   */
  const handlePreviewTestPaper = useCallback(() => {
  ...
  return {
    ...
    getSelectedProblems,
    setTestCreatedCallback, // 시험 생성 완료 콜백 설정
  };
}
