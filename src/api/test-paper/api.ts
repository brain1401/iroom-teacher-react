 ...
 */
const testPaperApiClient = authApiClient.create({
  baseURL: authApiClient.defaults.baseURL + "/test-paper",
});
 ...
 */
async function testPaperApiRequest<T>(config: AxiosRequestConfig): Promise<T> {
  const response = await testPaperApiClient.request<T>(config);
...
export async function getUnitsByGrade(
  ...
  return testPaperApiRequest<UnitsByGradeData>({
    method: "GET",
    url: "/units",
    params: { grade },
